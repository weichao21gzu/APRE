...
class REPromptDataset(DictDataset):
    ...
    def get_labels(self, tokenizer):
        ...
        for name in self.temps:
            ...
            for index, temp in enumerate(self.temps[name]['temp']):
                ...
                original = tokenizer.encode(" ".join(['the', *temp]), add_special_tokens=False)[1:]
                final = tokenizer.encode(" ".join(['the', *_temp]), add_special_tokens=False)[1:]
    ...
    def list2tensor(self, data, tokenizer):
        ...
        for index, i in enumerate(tqdm(data)):
            ...
            if len(mask_pos) < 3:
                ...
                while True:
                    if i > 0:
                        if input_ids[j] != tokenizer.mask_token_id:
        ...
        print('odd_data: {}'.format(odd_data))
        for key in res:
    ...
    def tokenize(self, item, tokenizer):
        ...
        for i_t, token in enumerate(sentence):
            ...
            if i_t == int(pos_head['pos'][0]):
                tokens_wordpiece =  tokens_wordpiece
            if i_t == int(pos_head['pos'][1]):
                tokens_wordpiece =  [tokenizer.mask_token] + tokens_wordpiece
            if i_t == int(pos_tail['pos'][0]):
                tokens_wordpiece =  tokens_wordpiece
            if i_t == int(pos_tail['pos'][1]):
                tokens_wordpiece = [tokenizer.mask_token] + tokens_wordpiece
            sents.extend(tokens_wordpiece)

        # for i_t, token in enumerate(sentence):
        #     tokens_wordpiece = tokenizer.tokenize(token)
        #     if i_t + 1 == int(pos_head['pos'][0]):
        #         tokens_wordpiece = tokens_wordpiece
        #     if i_t + 1 == int(pos_head['pos'][1]):
        #         tokens_wordpiece = ['(', tokenizer.mask_token, ')'] + tokens_wordpiece
        #     if i_t + 1 == int(pos_tail['pos'][0]):
        #         tokens_wordpiece = tokens_wordpiece
        #     if i_t + 1 == int(pos_tail['pos'][1]):
        #         tokens_wordpiece = ['(', tokenizer.mask_token, ')'] + tokens_wordpiece
        #     sents.extend(tokens_wordpiece)
        ...
        sentence = tokenizer.convert_tokens_to_ids(sents)
        ...
        # e1 = tokenizer.encode(" ".join(['was', 'head_entity']), add_special_tokens=False)[1:]
        # e2 = tokenizer.encode(" ".join(['was', 'tail_entity']), add_special_tokens=False)[1:]

        '''
        prompt = ["The", "relation", "between", "the", *pos_head['name'], "and", "the", *pos_tail['name'], "is"]
        prompt = tokenizer.convert_tokens_to_ids(prompt)
        prompt = prompt + self.temp_ids[rel_name]['mask_ids'][2]
        '''
        ...
        # prompt = e1 + self.temp_ids[rel_name]['mask_ids'][2] + e2
        # prompt = self.temp_ids[rel_name]['mask_ids'][2]
